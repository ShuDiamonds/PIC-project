...
*　USB接続 汎用Ｉ／Ｏプログラム
*   柏野作の汎用USBクラスを使用
*   各々２つのパイプを持つインターフェースを２つに増やしたバージョン
*	USBフレ-ムワーク修正情報
*　　コンフィギュレーションの設定変更
...
*　　user.cの変更　　実行内容削除
*　　usbcfg.hの変更　IOモニタ削除
*　　usbdsc.c、usbdsc.h　デスクリプタ変更
*    usbgen.c、usbgen.h　エンドポイント２用追加
*    uabmmap.c、usbmmap.h　USB用バッファの追加　バンク配置変更
*
***************************************************************
...
void main(void)
{
    ...
    while(1)
    {
		...
		if((usb_device_state >= CONFIGURED_STATE)&&(UCONbits.SUSPND==0))
		{
			...
			/// 受信チェック
			RcvSize = USBIORead((byte*)&input_buffer,sizeof(input_buffer));		if(RcvSize)										// データ受信ありか？
    			{
            		LATCbits.LATC0 = !LATCbits.LATC0;				// LED0反転目印用
	    			switch(input_buffer[0])						// 最初の１文字チェック
	    			{
		    			case '0':								// メッセージ返送
         				if(!mUSBIOTxIsBusy())						// 送信レディーか？         			
           			 		USBIOWrite((byte*)MsgOK, 3);		// 送信実行
           			 	break;		    				
		    			case '1':								// 
            				LATCbits.LATC1 = !LATCbits.LATC1;		// LED1反転
            				if(LATCbits.LATC1 == 1)
