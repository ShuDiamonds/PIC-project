...
//// バンク変更　あらたに追加
#pragma udata usbram5=0x500     //See Linker Script,usb5:0x500-0x5FF(256-byte)

volatile far byte usbgen_out[USBGEN_EP_SIZE];
volatile far byte usbgen_in[USBGEN_EP_SIZE];

/// 追加インターフェース用バッファ追加
volatile far byte usbio_out[USBIO_EP_SIZE];
volatile far byte usbio_in[USBIO_EP_SIZE];
