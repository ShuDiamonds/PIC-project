...
{   
	...
	/// 新規インターフェース用追加
	USBIO_IO_OUT.Cnt = sizeof(usbio_out);
	USBIO_IO_OUT.ADR = (byte*)&usbio_out;
	USBIO_IO_OUT.Stat._byte = _USIE|_DAT0|_DTSEN;

	USBIO_IO_IN.ADR = (byte*)&usbio_in;
	USBIO_IO_IN.Stat._byte = _UCPU|_DAT1;
...
///
/// 新規インターフェース用Write関数追加
///
void USBIOWrite(byte *buffer, byte len)
{
	byte i;
...
/// 新規インターフェース用Read関数追加
///
byte USBIORead(byte *buffer, byte len)
{
    usbio_io_len = 0;
    ...
    if(!mUSBIORxIsBusy())
    {
         ...
         * the actual number of bytes received.
         */
        if(len > USBIO_IO_OUT.Cnt)
            len = USBIO_IO_OUT.Cnt;
        
        /*
         * Copy data from dual-ram buffer to user's buffer
         */
        for(usbio_io_len = 0; usbio_io_len < len; usbio_io_len++)
            buffer[usbio_io_len] = usbio_out[usbio_io_len];

        /*
         * Prepare dual-ram buffer for next OUT transaction
         */
        USBIO_IO_OUT.Cnt = sizeof(usbio_out);
        mUSBBufferReady(USBIO_IO_OUT);
    }//end if
