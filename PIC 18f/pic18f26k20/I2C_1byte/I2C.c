...
void main(void)						// メイン関数
{
	...
	//ローカル変数定義
		char demo[17] = "jyj\n\r";
		char Memodata[20] = "EEPROM Memory!!!"; //書き込みデータ
		char buffer[17]="                ";		//読み出しバッファ
		char i2c_data[20]="                 ";		//読み出しバッファ
	...
	***********************/
	...
	while(memadrs <= 0x7FFF)	  //メモリアドレスの最後まで繰り返し
	{
		temp = EByteWrite(0b1010000, memadrs, test);	 //書き込み
		if (temp < 0) 
		{							//エラーチェック
			itoa((-temp) , ermes);
			printf("Error");
		...
		wait_ms(12);				//書き込み動作待ち12msec
		
		itoa(memadrs, buffer);			//アドレス文字列に変換
		printf("read  adress = %s \n\r",buffer);				//アドレス表示
 
		data = EByteRead(0b10101000,memadrs); //データ読み出し
		if ( data < 0) 
		{				//エラーチェック
			itoa(0-data , ermes);
			printf("Error");
		...
		printf("data = %s \n\r",i2c_data);					//読み出しデータ表示					//読み出しデータ表示
		memadrs = memadrs+1;			//アドレス＋１
		test = test+1;					// Write data update
	}
